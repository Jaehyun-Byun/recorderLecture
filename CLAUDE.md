...
마이크 → useSpeechRecognition (Web Speech API) → onFinalSentence(문장)
   ...
   문장 = 세그먼트 즉시 done   문장 = 세그먼트 → 브라우저 번역     문장을 버퍼에 모음
                            (lib/translator.ts, 온디바이스)     → 3분마다 한 덩어리 flush(시간 기준)
                                                                → 세그먼트 1개, 순차 처리
...
- **세그먼트** = 화면 표시 단위. transcribe/translate에선 문장, refine에선 3분 단위 덩어리.
...
| **refine (고급)** | 전사 오류 교정 + **이해하기 쉬운 한국어 설명글로 재구성** + 어려운 용어·개념 설명(개조식) | 제공자 키 필요 |

## refine 모드: AI 강의 노트 (기능 6)

번역이 아니라 **발화 내용 기반으로 새로 설명하듯** 재작성한다.

- **버퍼 → flush** (`useTranscript`): 확정 문장을 `bufferRef`에 모으고, **시간 기준(문장 수 아님)**으로
  flush. 버퍼의 첫 문장이 들어오는 순간 **3분 타이머**(`CHUNK_INTERVAL_MS`)가 시작됨. 3분 뒤:
  - 문장이 **5개 이상**(`MIN_SENTENCES_TO_FLUSH`) 쌓여 있으면 그대로 flush.
  - 너무 적으면(발화량이 적었던 구간) **자르지 않고 계속 듣기** — 30초(`EXTEND_CHECK_MS`)마다 재확인,
    창이 열린 지 10분(`MAX_WINDOW_MS`)이 지나면 양과 무관하게 강제 flush(안전망).
  - "중지"(App `finalize()`)는 이 기준과 무관하게 항상 즉시 flush.
...
- **refine = 번역이 아니라 "강의 설명 생성"** — 전사에 오인식·오류가 많아 그대로 옮기면 매끄럽지 못함.
  ...
  타겟은 한국 학생.
- **3분 단위 시간 기준 배칭 + 얇은 덩어리 방지** — 문장 하나씩 보내면 맥락 없고 토큰 낭비 + Gemini
  무료 한도(하루 20회) 순삭. 처음엔 문장 수 기준(5→12→6→10→20)으로 조정하다가, 발화 속도에 안
  흔들리게 **시간 기준(3분)**으로 변경 — 강의 1시간이 ~20호출. 단, 3분마다 칼같이 자르면 발화량이
  적은 구간(침묵 많은 강의)은 내용 없는 덩어리가 나올 수 있어 — 5문장 미만이면 자르지 않고 계속
  듣다가, 10분(안전망) 지나면 그때는 무조건 flush.
...
- refine 모드 결과는 **보통 3분 지연 + 노트 생성 시간** (3분 타이머로 모음). 발화량이 적은 구간은
  내용이 찰 때까지 최대 10분까지 늘어날 수 있음(안전망 `MAX_WINDOW_MS`). 실시간 자막 아님(의도된 것).
