...
- **버퍼 → flush** (`useTranscript`): 확정 문장을 `bufferRef`에 모으고, **시간 기준(문장 수 아님)**으로
  flush. 버퍼의 첫 문장이 들어오는 순간 **3분 타이머**가 시작되고, 3분 뒤 그때까지 쌓인 문장을 통째로
  한 덩어리로 flush(`CHUNK_INTERVAL_MS`) — 몇 문장이든 상관없음. "중지"(App `finalize()`)는 즉시 flush.
...
- **refine = 번역이 아니라 "강의 설명 생성"** — 전사에 오인식·오류가 많아 그대로 옮기면 매끄럽지 못함.
  ...
  타겟은 한국 학생.
- **3분 단위 시간 기준 배칭** — 문장 하나씩 보내면 맥락 없고 토큰 낭비 + Gemini 무료 한도(하루 20회) 순삭.
  처음엔 문장 수 기준(5→12→6→10→20)으로 조정하다가, 발화 속도에 안 흔들리게 **시간 기준(3분)**으로
  변경 — 강의 1시간이 ~20호출.
...
- refine 모드 결과는 **최대 3분 지연 + 노트 생성 시간** (3분 타이머로 모음). 실시간 자막 아님(컨셉상 의도된 것).
